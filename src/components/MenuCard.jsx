...
export default function MenuCard({ item, onAdd }) {
  return (
    <div className="group hover:scale-105 transition-all duration-300 backdrop-blur-md bg-white/10 border-white/20 overflow-hidden rounded-xl">
      ...
      <div className="p-4">
        <span className="mb-2 bg-gradient-to-r from-red-500 to-orange-600 text-white font-bold px-2 py-1 rounded">{item.category}</span>
        <h4 className="font-bold text-white mb-2 text-lg">{item.name}</h4>
        <div className="flex justify-between items-center">
          <span className="text-2xl font-black text-red-400">${item.price}</span>
